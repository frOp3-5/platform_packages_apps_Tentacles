...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("tentacles_lock")) {
            resId = R.string.lockscreen_lock_icon_tentacles;
    ...
    private void resizeCarbonLock() {
        Bitmap tentaclesLock = BitmapFactory.decodeResource(getResources(), R.drawable.tentacles_lock);
        if (tentaclesLock != null) {
            String path = null;
            int px = requestImageSize();
            tentaclesLock = Bitmap.createScaledBitmap(tentaclesLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "tentacles_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                tentaclesLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
